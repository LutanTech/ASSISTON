    ...
    (function() {
        ...
        console.log(token)
        if(!token){
            return console.log('Support link Incomplete')
        }
        ...
        style.textContent = `
            ...
            .as-header-text p { font-size: 12px; opacity: 0.8; margin: 0; }
            .as-close {
            ...
            .as-messages {
                ...
                background: #f9fafb; display: flex; flex-direction: column; gap: 16px;
            }
            .as-row { display: flex; width: 100%; }
            .as-row.user { justify-content: flex-end; }
            .as-bubble {
                max-width: 80%; padding: 10px 14px; font-size: 14px; line-height: 1.4;
            ...
            .as-typing { padding: 8px 16px; font-size: 12px; color: var(--text-light); font-style: italic; display: none; }
            .as-input-area { padding: 16px; background: var(--white); border-top: 1px solid #f3f4f6; }
            .as-actions { display: flex; gap: 12px; margin-bottom: 8px; }
            .as-action-btn { background: none; border: none; color: var(--text-light); cursor: pointer; }
            .as-input-wrap { display: flex; gap: 8px; }
            .as-input-wrap input {
                flex: 1; background: #f3f4f6; border: none;
                padding: 10px 16px; border-radius: 20px; outline: none; font-size: 14px;
            }
            .as-send {
            ...
            @media (max-width: 480px) { .as-window { width: calc(100vw - 48px); height: 70vh; } }
        `;
        ...
        container.innerHTML = `
            <div id="as-window" class="as-window">
                ...
                <div id="as-typing" class="as-typing">Assiston is typing...</div>
                <div id="as-emoji" class="as-emoji"></div>
                <div class="as-input-area">
                    <div class="as-actions">
                        <button type="button" id="as-emoji-btn" class="as-action-btn">
                            <svg width="20" height="20" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.828 14.828a4 4 0 01-5.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                        </button>
        ...
        emojis.forEach(e => {
            ...
            b.className = 'as-emoji-item';
            b.style.cssText = 'background:none; border:none; font-size:20px; cursor:pointer; padding:4px;';
            b.textContent = e;
            b.onclick = () => {
                document.getElementById('as-input').value += e;
        ...
        document.getElementById('as-form').onsubmit = async (e) => {
            ...
            await fetch(window.API_BASE + '/support', {
                method:'POST',
                headers:{
        ...
        document.getElementById('as-file').onchange = (e) => {
            ...
            const int = setInterval(() => {
                p += 10;
                fill.style.width = p + '%';
                document.getElementById('as-perc').textContent = p + '%';
                if(p >= 100) {
                    clearInterval(int);
                    ...
                    reader.onload = (ev) => {
                        ...
                        r.innerHTML = `<div class="as-bubble" style="padding:4px"><img src="${ev.target.result}" style="width:100%; border-radius:8px; max-height:200px; object-fit:cover;"></div>`;
                        msgs.appendChild(r);
